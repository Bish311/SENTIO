...
async def run_live_single_step_case(
    ...
    cid, now_dt = f"cust_demo_{str(ulid.ULID()).lower()[-6:]}", now_utc()
    sub_id, batch_id = f"sub_demo_{cid[-6:]}", f"batch_demo_{cid[-6:]}"
    ...
    ev1 = await append_event(
        ...
        {"subscription_id": sub_id, "amount_paise": sc["paise"], "decline_code": sc["code"]},
        batch_id=batch_id, dedup_key=f"fail_{sub_id}",
    )
    if ev1:
        await project_event(session, ev1)
    steps.append({
        "stage": "1. Spine Event Store", "actor": "system",
        "detail": f"Verified HMAC-SHA256. Appended event {ev1.id if ev1 else 0}.",
        "proof": {"hmac_valid": True, "event_type": "payment.failed", "paise": sc["paise"]},
    })

    case_obj = await handle_payment_failed(
        session, sub_id, sc["paise"], sc["code"], {"description": sc["desc"]}, "agent", batch_id
    )
    cause, conf, src, _ = await diagnose_payment_failure(
        session, case_obj.id, sc["code"], sc["desc"]
    )
    model_tag = settings.LLM_T1_MODEL if src == "llm" else "matrix"
    steps.append({
        "stage": "2. Lens Root-Cause Diagnostic", "actor": "lens",
        "detail": f"Diagnosed '{cause}' via {model_tag} with {conf:.2f} confidence.",
        "proof": {"source": src, "cause": cause, "confidence": conf, "model": model_tag},
    })

    intv_id = f"intv_{str(ulid.ULID()).lower()}"
    rcpt = await evaluate_proposal(
        session, case_obj.id, intv_id, "payment_link", sc["paise"], sc["paise"], now_dt, conf
    )
    steps.append({
    ...
    steps.append({
        ...
        "detail": f"Quiet hours checked. Window opens at {legal_dt.strftime('%H:%M')} IST.",
        "proof": {"scheduled_for": legal_dt.strftime("%d %b, %H:%M IST")},
    })

    await execute_reach_and_settle(session, case_obj, sc, cause, now_dt, batch_id, steps)
    await session.commit()
    return {
        "case_id": case_obj.id, "batch_id": batch_id,
        "steps": steps, "customer": sc["name"], "amount": sc["amount_str"],
    }
